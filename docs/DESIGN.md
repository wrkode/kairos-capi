...
- **Distribution validation**: `spec.distribution` must be `"k0s"` or `"k3s"`
- **Worker token requirement**:
  - k0s: `workerToken` or `workerTokenSecretRef` is required
  - k3s: `k3sToken`, `k3sTokenSecretRef`, `workerToken`, or `workerTokenSecretRef` is required
...
- `spec.distribution`: `"k0s"` | `"k3s"` (k3s currently supported for single-node control plane)
...
- `spec.k3sToken` / `spec.k3sTokenSecretRef`: k3s join token (k3s-specific, Secret ref preferred)
- `spec.manifests`: Kubernetes manifests to deploy via the distribution
...
The controller generates Kairos cloud-config using provider-specific templates and Kairos's native distribution integrations:

1. k0s:
   - `k0s:` block for control-plane nodes (with optional `--single` for single-node mode)
   - `k0s-worker:` block for worker nodes with token file
   - Worker tokens written to `/etc/k0s/token`
   - Manifests written to `/var/lib/k0s/manifests/`
2. k3s:
   - `k3s:` block for control-plane nodes (single-node server)
   - `k3s-agent:` block for worker nodes with token file and server URL
   - Worker tokens written to `/etc/rancher/k3s/token`
   - Manifests written to `/var/lib/rancher/k3s/server/manifests/`
...
2. **k3s HA**: k3s HA control plane is not implemented
...
2. **k3s HA**: Implement k3s HA control plane
