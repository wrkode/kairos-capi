...
5) Verify controller join token flow (k0s auto-generated):
```
clusterctl get kubeconfig kairos-cluster-ha > kairos-ha.kubeconfig
kubectl --kubeconfig kairos-ha.kubeconfig -n kairos-system get secret k0s-controller-join-token
kubectl get secret -n default kairos-cluster-ha-k0s-controller-join-token
```
Expected:
- Workload Secret exists in `kairos-system` with key `token`.
- Mirrored Secret exists in management cluster namespace.
...
4) Verify controller join token flow (k0s auto-generated):
```
clusterctl get kubeconfig kairos-cluster-kv-ha > kairos-kv-ha.kubeconfig
kubectl --kubeconfig kairos-kv-ha.kubeconfig -n kairos-system get secret k0s-controller-join-token
kubectl get secret -n default kairos-cluster-kv-ha-k0s-controller-join-token
```
...
- Controller join tokens are generated by k0s on the init node; users do not provide them for k0s HA.
- Installer-based Kairos boot flows require the top-level `install:` block in cloud-config for CAPV/CAPK.
- CAPV/CAPK samples set `install.auto=true` by default for installer-based flows.